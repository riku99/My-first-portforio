...
describe "Users", type: :system do
  ...
  it "管理者ユーザは他のユーザを削除することができること", retry: 5 do
    test_log_in(user)
  ...
  it "showページのフィードが選択に応じて変わること" do
    test_log_in user
    discovery = FactoryBot.create(:discovery, user: user, content: "userの投稿です")
    comment = FactoryBot.create(:comment, user: user, content: "userのコメントです")
    visit current_path
    expect(page).to have_content "userの投稿です"
    expect(page).to_not have_content "userのコメントです"
    click_link "Comment"
    expect(page).to have_content "userのコメントです"
    expect(page).to_not have_content "userの投稿です"
    user.favo(discovery)
    click_link "Favorite"
    expect(page).to have_content "userの投稿です"
  end
